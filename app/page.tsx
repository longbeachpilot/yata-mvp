...
export default function HomePage(){
 ...
 <section className="section container splitBanner"><div><span className="eyebrow">YA TA LOGBOOK</span><h2>연수할수록 쌓이는<br/>나만의 운전 기록</h2><p>교관이 수업 내용, 숙련도와 다음 목표를 기록합니다. 교관이 바뀌어도 학습 흐름을 이어갈 수 있습니다.</p><Link className="textLink" href="/logbook">내 Logbook 보기 <ArrowRight size={16}/></Link></div><div className="progressCard"><div className="progressTop"><strong>Logbook 예시</strong><span>DEMO</span></div>{[['기본 조작',100],['차선 변경',80],['주차',60],['고속도로',40]].map(([x,p])=><div className="skill" key={x as string}><div><span>{x}</span><b>{p}%</b></div><div className="bar"><i style={{width:`${p}%`}}/></div></div>)}</div></section></>
}
